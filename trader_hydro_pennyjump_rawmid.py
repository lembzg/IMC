...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        result["HYDROGEL_PACK"], shared = self.hydro(state, shared)

        traderData = json.dumps(shared)
    ...
    def hydro(self, state: TradingState, shared: dict):
        ...
        if buy_room > 0 and bid_price <= fair_value - edge:
            if trend > -trend_thresh:
                orders.append(Order(product, int(bid_price), min(quote_size, buy_room)))

        if sell_room > 0 and ask_price >= fair_value + edge:
            if trend < trend_thresh:
                orders.append(Order(product, int(ask_price), -min(quote_size, sell_room)))
