...
class Trader:
    ...
    ACO_POS_LIMIT     = 80
    ACO_ROLL_WINDOW   = 50
    ACO_SIZE_LO       = 20     # size when abs_ratio < 0.5
    ACO_SIZE_MID      = 15     # size when abs_ratio < 0.75
    ACO_SIZE_HI       = 6      # size when abs_ratio >= 0.75
    ACO_INV_MULT      =  4     # inventory skew: fv -= round(ratio * X)
    ...
    def ash(self, state: TradingState, sd: dict) -> tuple:
        ...
        # Inventory skew
        ratio = pos / pos_lim
        fv -= round(ratio * self.ACO_INV_MULT)
        fv = round(fv)
