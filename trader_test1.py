...
PEBBLES_TRADES = ("PEBBLES_XL",)  # XS removed: XS-XL spread trends hard, separate XS strategy
POSITION_LIMIT = 10
HISTORY_LIMIT = 200
PRODUCT_OFFSETS = {
...
EXIT_THRESHOLD = 105
ORDER_SIZE = 4
PEBBLES_PAIR_FAIR = 20721  # actual mean of XS+XL sum over Round 5 data
...
CHOCOLATE_SIGNAL_ANCHOR = 19875
CHOCOLATE_SIGNAL_ENTRY = 100
CHOCOLATE_SIGNAL_EXIT = 75
CHOCOLATE_SIGNAL_SIZE = 1
...
IRON_PEB_ENTRY = 250
IRON_PEB_EXIT = 0
PEBBLES_L_FAIR_PRODUCTS = ("PEBBLES_XS", "PEBBLES_S", "PEBBLES_M", "PEBBLES_XL")  # XS included: improves signal (std 916 vs 1061 without)
PEBBLES_L_ENTRY_THRESHOLD = 1400
PEBBLES_L_EXIT_THRESHOLD = 300
CHIP_CT_TRADES = ("MICROCHIP_CIRCLE", "MICROCHIP_TRIANGLE")
CHIP_CT_PAIR_FAIR = 19127
CHIP_CT_ENTRY = 200
CHIP_CT_EXIT = 100
ROBOT_LD_TRADES = ("ROBOT_LAUNDRY", "ROBOT_DISHES")
ROBOT_LD_PAIR_FAIR = 19600
ROBOT_LD_ENTRY = 150
ROBOT_LD_EXIT = 75
...
NYLON_RED_FAIR_SPREAD = -700
NYLON_RED_MARGIN = 75
# ─── Robot Vacuuming (group mean vs LAUNDRY, DISHES, IRONING, MOPPING) ───
ROBOT_VAC_TARGET = "ROBOT_VACUUMING"
ROBOT_VAC_GROUP = ("ROBOT_LAUNDRY", "ROBOT_DISHES", "ROBOT_IRONING", "ROBOT_MOPPING")
ROBOT_VAC_FAIR_DIFF = -1190.9  # VAC - group_avg mean
ROBOT_VAC_ENTRY = 200          # ~1.4σ of 143.45
ROBOT_VAC_EXIT = 75
ROBOT_VAC_SIZE = 10
...
PANEL2X4_EXIT = 100
PANEL2X4_SIZE = 10
# ─── Panel 1X4 (vs Panel 2X2; 1X4−2X2 mean=−218, std=193) ───
PANEL1X4_TARGET = "PANEL_1X4"
PANEL1X4_SIGNAL = "PANEL_2X2"
PANEL1X4_FAIR_DIFF = -218.1     # 1X4 - 2X2 mean
PANEL1X4_ENTRY = 260            # ~1.35σ of 193.10
PANEL1X4_EXIT = 75
PANEL1X4_SIZE = 10
# ─── Pebbles XS standalone (vs Pebbles S; XS−S mean=−1893, std=368) ───
PEBBLES_XS_TARGET = "PEBBLES_XS"
PEBBLES_XS_SIGNAL = "PEBBLES_S"
PEBBLES_XS_FAIR_DIFF = -1893.0  # XS - S mean
PEBBLES_XS_ENTRY = 500          # ~1.36σ of 367.96
PEBBLES_XS_EXIT = 150
PEBBLES_XS_SIZE = 10
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        result.update(self._trade_robot_vacuuming(state))
        ...
        result.update(self._trade_pebbles_xs(state))
    ...
    def _trade_nylon_red_signal(self, state: TradingState) -> Dict[str, List[Order]]:
        nylon_od = state.order_depths.get("SLEEP_POD_NYLON")
        red_od = state.order_depths.get("UV_VISOR_RED")
        if not nylon_od or not nylon_od.buy_orders or not nylon_od.sell_orders:
            return {"SLEEP_POD_NYLON": []}
        if not red_od or not red_od.buy_orders or not red_od.sell_orders:
            return {"SLEEP_POD_NYLON": []}

        nylon_bid = max(nylon_od.buy_orders)
        nylon_ask = min(nylon_od.sell_orders)
        red_mid = (max(red_od.buy_orders) + min(red_od.sell_orders)) / 2.0
        nylon_mid = (nylon_bid + nylon_ask) / 2.0
        spread = nylon_mid - red_mid

        position = state.position.get("SLEEP_POD_NYLON", 0)
        buy_room = POSITION_LIMIT - position
        sell_room = POSITION_LIMIT + position
        orders: List[Order] = []

        if spread < NYLON_RED_FAIR_SPREAD - NYLON_RED_MARGIN and buy_room > 0:
            qty = min(buy_room, -nylon_od.sell_orders[nylon_ask])
            if qty > 0:
                orders.append(Order("SLEEP_POD_NYLON", nylon_ask, qty))
        elif spread > NYLON_RED_FAIR_SPREAD + NYLON_RED_MARGIN and sell_room > 0:
            qty = min(sell_room, nylon_od.buy_orders[nylon_bid])
            if qty > 0:
                orders.append(Order("SLEEP_POD_NYLON", nylon_bid, -qty))

        return {"SLEEP_POD_NYLON": orders}
    ...
    def _trade_polyester_mm(self, state: TradingState, histories: dict) -> Dict[str, List[Order]]:
        ...
        hist = histories.setdefault("poly_mid", [])
        hist.append(mid)
        if len(hist) > 50:
            del hist[:len(hist) - 50]
        fair = round(sum(hist) / len(hist))
        pos = state.position.get(product, 0)
        buy_cap = POSITION_LIMIT - pos
        sell_cap = POSITION_LIMIT + pos
        skew = -1 if pos > 4 else (1 if pos < -4 else 0)
        bid_px = min(fair - 2 + skew, asks[0] - 1)
        ask_px = max(fair + 2 + skew, bids[0] + 1)
        orders: List[Order] = []
        if buy_cap > 0:
            orders.append(Order(product, int(bid_px), min(2, buy_cap)))
        if sell_cap > 0:
            orders.append(Order(product, int(ask_px), -min(2, sell_cap)))
    ...
    def _trade_robot_ld_pair(self, state: TradingState) -> Dict[str, List[Order]]:
        depths = {}
        mids = {}
        ...
        for product in ROBOT_LD_TRADES:
            ...
            if position > 0 and ROBOT_LD_PAIR_FAIR - ROBOT_LD_EXIT <= pair_sum <= ROBOT_LD_PAIR_FAIR + ROBOT_LD_EXIT and sell_room > 0:
                orders.append(Order(product, best_bid, -min(sell_room, position)))
            elif position < 0 and ROBOT_LD_PAIR_FAIR - ROBOT_LD_EXIT <= pair_sum <= ROBOT_LD_PAIR_FAIR + ROBOT_LD_EXIT and buy_room > 0:
                orders.append(Order(product, best_ask, min(buy_room, -position)))

            if pair_sum <= ROBOT_LD_PAIR_FAIR - ROBOT_LD_ENTRY and buy_room > 0:
                orders.append(Order(product, best_ask, buy_room))
            elif pair_sum >= ROBOT_LD_PAIR_FAIR + ROBOT_LD_ENTRY and sell_room > 0:
                orders.append(Order(product, best_bid, -sell_room))
    ...
    def _trade_chocolate_signal(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        residual = chocolate_mid + vanilla_mid - CHOCOLATE_SIGNAL_ANCHOR
        ...
        if position > 0 and residual >= CHOCOLATE_SIGNAL_EXIT and sell_room > 0:
            quantity = min(CHOCOLATE_SIGNAL_SIZE, sell_room, position)
            orders.append(Order("SNACKPACK_CHOCOLATE", best_bid, -quantity))
            sell_room -= quantity

        if position < 0 and residual <= -CHOCOLATE_SIGNAL_EXIT and buy_room > 0:
            quantity = min(CHOCOLATE_SIGNAL_SIZE, buy_room, -position)
            orders.append(Order("SNACKPACK_CHOCOLATE", best_ask, quantity))
            buy_room -= quantity

        if residual <= -CHOCOLATE_SIGNAL_ENTRY and buy_room > 0:
            quantity = min(CHOCOLATE_SIGNAL_SIZE, buy_room)
            orders.append(Order("SNACKPACK_CHOCOLATE", best_ask, quantity))
            buy_room -= quantity

        if residual >= CHOCOLATE_SIGNAL_ENTRY and sell_room > 0:
            quantity = min(CHOCOLATE_SIGNAL_SIZE, sell_room)
            orders.append(Order("SNACKPACK_CHOCOLATE", best_bid, -quantity))
            sell_room -= quantity
    ...
    def _trade_signal_target_pair(
        ...
        """Generic: trade TARGET when (TARGET_mid - SIGNAL_mid) deviates from fair_diff."""
    ...
    def _trade_group_mean_target(
        ...
        """Generic: trade TARGET when (TARGET_mid - group_avg) deviates from fair_diff."""
        group_mids: List[float] = []
    ...
    def _trade_robot_vacuuming(self, state: TradingState) -> Dict[str, List[Order]]:
        return self._trade_group_mean_target(
            state, ROBOT_VAC_TARGET, ROBOT_VAC_GROUP,
            ROBOT_VAC_FAIR_DIFF, ROBOT_VAC_ENTRY, ROBOT_VAC_EXIT, ROBOT_VAC_SIZE)
    ...
    def _trade_panel_1x4(self, state: TradingState) -> Dict[str, List[Order]]:
        return self._trade_signal_target_pair(
            state, PANEL1X4_SIGNAL, PANEL1X4_TARGET,
            PANEL1X4_FAIR_DIFF, PANEL1X4_ENTRY, PANEL1X4_EXIT, PANEL1X4_SIZE)

    def _trade_pebbles_xs(self, state: TradingState) -> Dict[str, List[Order]]:
        """Trade PEBBLES_XS independently vs PEBBLES_S signal."""
        return self._trade_signal_target_pair(
            state, PEBBLES_XS_SIGNAL, PEBBLES_XS_TARGET,
            PEBBLES_XS_FAIR_DIFF, PEBBLES_XS_ENTRY, PEBBLES_XS_EXIT, PEBBLES_XS_SIZE)
    ...
    def _trade_pebbles_pair(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        # Only XL remains in PEBBLES_TRADES; XS is handled by _trade_pebbles_xs
        # XL standalone mean-reversion: actual data mean = 14,665
        XL_FAIR = 14665
        XL_ENTRY = 400
        result: Dict[str, List[Order]] = {}
        ...
        for product in PEBBLES_TRADES:
            ...
            if position > 0 and mid >= XL_FAIR and sell_room > 0:
                quantity = min(PEBBLES_ORDER_SIZE, sell_room, position)
                orders.append(Order(product, best_bid, -quantity))
                sell_room -= quantity

            if position < 0 and mid <= XL_FAIR and buy_room > 0:
                quantity = min(PEBBLES_ORDER_SIZE, buy_room, -position)
                orders.append(Order(product, best_ask, quantity))
                buy_room -= quantity

            if mid <= XL_FAIR - XL_ENTRY and buy_room > 0:
                quantity = min(PEBBLES_ORDER_SIZE, buy_room)
                orders.append(Order(product, best_ask, quantity))
                buy_room -= quantity

            if mid >= XL_FAIR + XL_ENTRY and sell_room > 0:
