...
class Trader:
    ...
    def ash(self, state: TradingState, shared: dict):
        ...
        base_pos_lim   = pos_lim - 1  # baseline gets 79 (max safe with base_pos accounting)
        quote_lim      = 10
        sma_window     = 5
        entry_size     = 1
        long_entry_px  = 10003
        ...
        ov_orders = []
        day_end = state.timestamp >= 999500  # force-close in last 500 ticks

        # Active exit: hit best_bid when market moved 2+ ticks above entry, or day ending
        if ov["long_inv"] > 0:
            if best_bid >= long_entry_px + 2 or day_end:
                ov_orders.append(Order(product, best_bid, -ov["long_inv"]))
                ov["long_exit_px_pend"]  = best_bid
                ov["long_exit_qty_pend"] = ov["long_inv"]
        else:
            # only enter long when baseline is already net short (overlay counteracts skew)
            if not day_end and base_pos < -10:
        ...
        # Active exit: hit best_ask when market moved 2+ ticks below entry, or day ending
        if ov["short_inv"] > 0:
            if best_ask <= short_entry_px - 2 or day_end:
                ov_orders.append(Order(product, best_ask, ov["short_inv"]))
                ov["short_exit_px_pend"]  = best_ask
                ov["short_exit_qty_pend"] = ov["short_inv"]
        else:
            # only enter short when baseline is already net long (overlay counteracts skew)
            if not day_end and base_pos > 10:
                if sz := min(entry_size, max(0, pos_lim + pos)):
