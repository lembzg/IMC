...
class Trader:
    def run(self, state: TradingState):
        ...
        ov = {}
        if state.traderData:
            try: ov = json.loads(state.traderData).get("ov", {})
            except: pass

        result['ASH_COATED_OSMIUM'], ov = self.ash(state, ov)
        traderData = json.dumps({"ov": ov})
        logger.flush(state, result, conversions, traderData)
        return result, conversions, traderData

    def ash(self, state, ov):
        ...
        ov.setdefault("long_inv",  0); ov.setdefault("long_entry_qty_pend",  0); ov.setdefault("long_entry_px_pend",  0)
        ov.setdefault("short_inv", 0); ov.setdefault("short_entry_qty_pend", 0); ov.setdefault("short_entry_px_pend", 0)
        ...
        if product not in state.order_depths: return result, ov
        od = state.order_depths[product]
        bids = sorted(od.buy_orders, reverse=True)
        asks = sorted(od.sell_orders)
        if not bids or not asks: return result, ov

        best_bid = bids[0]; best_ask = asks[0]
        pos = state.position.get(product, 0)

        seen = list(ov["seen"]); seen_set = set(seen)
        for t in state.own_trades.get(product, []):
            ...
            px = int(t.price); qty = abs(int(t.quantity))
            is_buy = (t.buyer == "SUBMISSION")
            if is_buy:
                if ov["long_entry_qty_pend"] > 0 and px == ov["long_entry_px_pend"]:
                    ov["long_inv"] += min(qty, ov["long_entry_qty_pend"])
                    ov["long_entry_qty_pend"] -= min(qty, ov["long_entry_qty_pend"])
                ...
                if ov["short_entry_qty_pend"] > 0 and px == ov["short_entry_px_pend"]:
                    ov["short_inv"] += min(qty, ov["short_entry_qty_pend"])
                    ov["short_entry_qty_pend"] -= min(qty, ov["short_entry_qty_pend"])
        ...
        ov["seen"] = seen[-200:]
        ov["long_entry_px_pend"] = ov["long_entry_qty_pend"] = 0
        ov["long_exit_px_pend"]  = ov["long_exit_qty_pend"]  = 0
        ov["short_entry_px_pend"]= ov["short_entry_qty_pend"]= 0
        ov["short_exit_px_pend"] = ov["short_exit_qty_pend"] = 0

        # Long exit — active take-profit: hit bid when it rises 2+ ticks above entry
        if ov["long_inv"] > 0 and best_bid >= long_entry_px + 2:
            result.append(Order(product, best_bid, -ov["long_inv"]))
            ov["long_exit_px_pend"]  = best_bid
            ov["long_exit_qty_pend"] = ov["long_inv"]

        # Short exit — active take-profit: hit ask when it falls 2+ ticks below entry
        elif ov["short_inv"] > 0 and best_ask <= short_entry_px - 2:
            result.append(Order(product, best_ask, ov["short_inv"]))
            ov["short_exit_px_pend"]  = best_ask
        ...
        # Short entry
        if ov["short_inv"] == 0 and ov["long_inv"] == 0:
            room = max(0, pos_lim + pos)
            sz = min(entry_size, room)
        ...
        logger.print("l_inv", ov["long_inv"], "s_inv", ov["short_inv"], "bid", best_bid, "ask", best_ask)
        return result, ov
