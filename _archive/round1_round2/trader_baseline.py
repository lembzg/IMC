...
class Trader:
    ...
    def ash(self, state: TradingState, shared: dict):
        ...
        imbalance_skew=shared.get("imbalance_skew", 0.0)
        imbalance_ttl=shared.get("imbalance_ttl", 0)
        if 'ASH_COATED_OSMIUM' not in state.order_depths:
            return result, {"bid_hist": bid_hist, "ask_hist": ask_hist, "imbalance_skew": imbalance_skew, "imbalance_ttl": imbalance_ttl}
        ...
        if len(ask_hist) > sma_window:
            ask_hist.pop(0)
        if curr_best_ask is None and curr_best_bid is not None:
            imbalance_skew=0.25
            imbalance_ttl=2
        elif curr_best_bid is None and curr_best_ask is not None:
            imbalance_skew=-0.25
            imbalance_ttl=2
        elif imbalance_ttl > 0:
            imbalance_ttl-=1
        else:
            imbalance_skew*=0.75
            if -0.05 < imbalance_skew < 0.05:
                imbalance_skew=0.0
        ...
        fair_value = (bid_sma + ask_sma) / 2.0 + imbalance_skew
        ...
        if fair_value is not None:
            ...
            sell_threshold = fair_value - (1 if pos > 15 else 0) - 1
            for ask_px in asks:
