...
class Trader:
    ...
    def ash(self, state: TradingState, sd: dict) -> tuple:
        ...
        # ---- OBI spike guard ----
        # Only skew FV when OBI is in the slow-drift range (not post-fill spikes)
        bid_depth = sum(bids.values())
        ask_depth = sum(abs(asks[p]) for p in asks)
        obi = (bid_depth - ask_depth) / (bid_depth + ask_depth + 1e-9)
        if abs(obi) < 0.8:
            fv += round(obi * 2)  # max ±1-2 tick skew
    ...
    def pepper(self, state: TradingState):
        ...
        qty = pos_lim - pos
        if qty > 0:
            return [Order(product, asks[0], qty)]
        return []