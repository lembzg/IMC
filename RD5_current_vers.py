...
class Trader:
    ...
    def _trade_pebbles_ms_pair(self, state: TradingState) -> Dict[str, List[Order]]:
        # Signal needs both M and S mids; orders only for M (S is owned by _trade_iron_peb_pair).
        mids = {}
        m_depth = None
        for product in PEBBLES_MS_TRADES:
            ...
            if order_depth is None or not order_depth.buy_orders or not order_depth.sell_orders:
                return {"PEBBLES_M": []}
            mids[product] = (max(order_depth.buy_orders.keys()) + min(order_depth.sell_orders.keys())) / 2.0
            if product == "PEBBLES_M":
                m_depth = order_depth

        pair_mid = mids["PEBBLES_M"] + mids["PEBBLES_S"]
        pair_fair = PEBBLES_MS_PAIR_FAIR

        best_bid = max(m_depth.buy_orders.keys())
        best_ask = min(m_depth.sell_orders.keys())
        position = state.position.get("PEBBLES_M", 0)
        buy_room = POSITION_LIMIT - position
        sell_room = POSITION_LIMIT + position
        orders: List[Order] = []
        ...
        if position > 0 and pair_mid >= pair_fair + PEBBLES_MS_EXIT_THRESHOLD and sell_room > 0:
            ...
            orders.append(Order("PEBBLES_M", best_bid, -quantity))
            sell_room -= quantity
        ...
        if position < 0 and pair_mid <= pair_fair - PEBBLES_MS_EXIT_THRESHOLD and buy_room > 0:
            ...
            orders.append(Order("PEBBLES_M", best_ask, quantity))
            buy_room -= quantity
